 ...
 */
public class GraphViewweek extends View {
    ...
    public GraphViewweek(Context context, double[] values, String title, String[] horlabels, String[] verlabels, boolean type) {
        super(context);
        if (values == null)
            values = new double[0];
        else
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i <= horlabels.length; i++) { //날짜 기입
            ...
            paint.setColor(Color.BLACK);
            if (i != horlabels.length)
                canvas.drawText(horlabels[i], x + 75, height - 20, paint);
        }
